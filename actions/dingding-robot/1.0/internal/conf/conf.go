...
// Conf action 入参
type Conf struct {
	Msg        string   `env:"ACTION_MSG" required:"true"`
	RobotToken []string `env:"ACTION_TOKEN" required:"true"`
	KeyWord    string   `env:"ACTION_KEYWORD" required:"true"`
	// env
...
func RobotToken() []string {
	return cfg.RobotToken
}
...
func DiceVersion() string {
	return cfg.DiceVersion
}
